...
/*   Updated: 2022/03/07 15:25:09 by nfaivre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((position.x % (double)1) > (double)0 && (position.y % (double)1) > (double)0)
		return (false);
	if ((position.y % (double)1) > (double)0)
	{
		if (map[(int)(position.y / (double)1)][(int)position.x] == '1'
			|| map[(int)(position.y / (double)1)][(int)position.x - 1] == '1')
			return (true);
	}
	else if ((position.x % (double)1) > (double)0)
	{
...
static void	get_next_intersection(t_position *position, double angle)
{
	...
	else if ((position.y % (double)1) > (double)0)
	{
		plan_start.x = position->x - (double)1;
...
double	get_wall_distance(t_position player_position, double angle, t_map *map)
{
	t_position	current_position;

	current_position.x = player_position.x;
	current_position.y = player_position.y;
	while (!does_position_touch_a_wall(current_position, map->content))
	{
